...
def generate_blockchain_configurations():
    ...
    boot_lab.create_startup_file_from_string(
        ...
        f"--validators {num_validators} --members {num_members} --bootnodes 0 --outputPath '/shared'"
    )
    ...
    with open('./shared.tar', 'wb') as f:
        bits, stat = bootnode.api_object.get_archive('/shared')
        for chunk in bits:
            f.write(chunk)
    ...
    with tarfile.open('shared.tar') as f:
        f.extractall(path=configurations_path)

    os.remove('shared.tar')
...
def generate_config(configurations_path: str, parameters: list[str]):
    ...
    nodes_dir = list(filter(lambda x: "validator" in x, os.listdir(configurations_path)))
    sorted_nodes = sorted(nodes_dir, key=lambda item: (
    ...
    contract_src_dir = os.path.abspath('shared/contract')

    for node_name in sorted_nodes:
        logger.info(f"Configuring node {node_name}...")
        
        target_dir = os.path.join(node_name, "data")
    ...
    configure_leader("validator0", members_for_group, parameters, node_metadata)

    logger.info("Network configuration written to disk!")


def configure_leader(node_name, members_for_group, parameters, node_metadata):
    target_dir = os.path.join(node_name, "data", "contract")
    ...
    validators = [meta['address'] for name, meta in node_metadata.items() if "validator" in name]
    agent_ids = ", ".join(validators)
...
if __name__ == "__main__":
    try:
        ...
        generate_config(config_path, ["P_0_0", "P_0_1", "P_0_2", "P_0_3", "P_0_4"])
